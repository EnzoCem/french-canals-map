...
| ~~Lock waypoints from IENC~~ | Done | 186 locks imported from `data/ienc_locks.geojson` into `WAYPOINTS` as `lk100..lk285` (2026-04-24). Each carries chamber length × width in the description so the vessel-profile filter can flag locks that won't fit. Coverage: Seine 41, Moselle 33, Dunkerque-Escaut 31, Seine-Amont 26, Saône 19+7, Rhine 18, Oise 13, Rhône au Rhin 2, Leie 1. 86 % land within 500 m of the nearest waterway; the remaining 14 % are trans-border locks (Saar) whose canals aren't in our geojson yet. |
