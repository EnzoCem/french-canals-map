...
| ~~Bridge height markers~~ | Done | 🌉 Bridges layer renders 3,198 bridges across FR/NL/DE/BE/AT/CH waterways (Rhine, Moselle, Seine, Saône, Oise, Garonne tidal, Dunkerque-Escaut, Maas, Waal, Main, Main-Donau-Kanal, Donau, Albertkanaal, Hochrhein, etc.). Per-bridge air clearance from official IENC cells (VNF, Rijkswaterstaat, WSV, + BE/AT/CH via the Inland ENC Europe 05.2022 bundles); vessel-profile-aware colouring (green/amber/red). Extraction pipeline: `extract_ienc.py` → `data/bridges.geojson`. |
...
| ~~IENC bridge air clearances~~ | Done | 🌉 Bridges layer — see row above. 3,198 bridges with VERCLR; vessel-profile-aware colouring. |
| ~~IENC locks reconciliation data~~ | Done | `data/ienc_locks.geojson` (792 locks with length/width/rise) + `bridges_locks_reconciliation.csv` for manual review. Source for cherry-picking lock waypoints missing from the app. |
| ~~IENC moorings reconciliation data~~ | Done | `data/ienc_moorings.geojson` (6,592 quays + pontoons) + `bridges_moorings_reconciliation.csv`. Surfaces candidate matches (e.g. Chalon port position verified; Seurre, Toul positions worth reviewing). |
| ~~IENC channel axis (🧭 Channel)~~ | Done | 7,242 dredged-channel centerline segments from `wtwaxs` layer. Toggleable polyline overlay (dashed, per-waterway colour) — shows the official navigation axis vs. OSM's river bank. Significant on meandering sections (Moselle, lower Seine). |
| ~~IENC obstructions (⚠ Hazards)~~ | Done | 2,112 navigation hazards from `OBSTRN` — rocks, snags, foul areas, islets. Colour-coded markers by water-level category (submerged/awash/visible). Popup shows CATOBS type + WATLEV context + bilingual description. |
...
| ~~IENC bundle expansion (Tier 1)~~ | Done (FR/NL/DE — BE/AT deferred) | 2026-04-25: ingested the user-supplied IENC France 2021 + Inland ENC Europe 05.2022 bundles. Net additions: full Rhône Lyon→Mediterranean (CNR cells `3T5RHO*`, 33 cells, ~88 new bridges + 4 locks), German Saar (10 cells, 16 locks), Belgium (8 zips, 380 bridges in Belgium-waterway category + 82 Albert Canal). Bridges geojson: 990 → 1,700 at the time. Locks: 192 → 325 in source data, 208 imported into WAYPOINTS. Totals after the NL (Rijkswaterstaat) + DE (WSV) ingestion: 2,485 bridges, 644 locks. |
| ~~IENC bundle expansion (Tier 2 — BE/AT/CH)~~ | Done | 2026-07-10: ingested the local Inland ENC Europe 05.2022 bundles for Belgium (Albertkanaal 82 bridges + 15 Flemish waterways via `7V7*` mappacks), Austria (Donau `2W7D*` base edition, ~106 bridges — Donau now 154 incl. AT; the 8 incremental update zips intentionally NOT applied, fresher data via viadonau/DoRIS if needed), Switzerland (Hochrhein 19 bridges), and restored the Rhône CNR cells via `ienc/Rhone_Lyon_Med.zip` (11 → 99 Rhône bridges). New totals: 3,198 bridges, 7,242 channel-axis segments, 2,112 obstructions, 792 locks, 6,592 moorings. SW bumped to fc-v17. |
| ~~IENC Danube extension (Wave 1 — SK/HU)~~ | Done | 2026-07-14: ingested the local Inland ENC Europe 05.2022 bundles for Slovakia (14 cell zips: Donau km 1709–1872 `2D7D####` + Gabčíkovo bypass `2D7DK###` → "Dunajský Kanál"; the ENC-SK.zip aggregate skipped as a duplicate) and Hungary (Donau km 1430–1810 `1H7D####` + Szentendrei-Duna `1H7SZD*`; the Tisza `HU_TI_*` bundle deliberately NOT fed in — no Tisza map geometry in scope). Donau bridges 154 → 229 (now AT + SK + HU). New totals: 3,276 bridges, 9,385 channel-axis segments, 2,112 obstructions, 798 locks, 7,083 moorings. Also: binary-junk text attributes in SK/HU cells are now dropped (surrogate guard in `_safe_str`), and `__MACOSX/._*.000` AppleDouble files are skipped when unpacking. Palette entries added for Dunajský Kanál + Szentendrei-Duna (channel-axis colouring). SW bumped to fc-v21. |
| ~~Danube Wave 2 (HR/RS/RO/BG — Budapest → Black Sea)~~ | Done | 2026-07-17: extended the app down the full navigable Danube to the Black Sea. Waterway geometry re-fetched with a Danube-specific clip bbox (Budapest → Sulina/Cernavodă) + the Canalul Dunăre-Marea Neagră (CDMN) to Constanța; curated routes 76 (Budapest–Belgrade), 77 (Belgrade–Cernavodă incl. the Iron Gates Đerdap I+II locks) and 78 (CDMN, Cernavodă–Constanța/Agigea); country wiring + authority links for 🇭🇷 Vodni putovi, 🇷🇸 Plovput, 🇷🇴 AFDJ/ACN, 🇧🇬 APPD/BULRIS; closure deep-links; corridor OSM POIs. IENC ingestion from the local Inland ENC Europe 05.2022 bundles: HR Dunav `5C7D####`, RS `2P7D####`, RO `3R7D*` (incl. Bala–Borcea) + CDMN `3R7DCC*`, BG `3B7D####` — Drava/Sava/Tisa and the RO `3R7PAM` branch deliberately NOT fed in. Donau bridges 229 → 271 (+ CDMN 21). New totals: 3,339 bridges, 9,620 channel-axis segments, 2,119 obstructions, 810 locks, 7,402 moorings. SW bumped to fc-v24. |
